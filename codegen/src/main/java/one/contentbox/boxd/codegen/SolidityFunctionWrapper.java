package one.contentbox.boxd.codegen;

import com.fasterxml.jackson.databind.ObjectMapper;
import com.squareup.javapoet.*;
import one.contentbox.boxd.abi.FunctionEncoder;
import one.contentbox.boxd.abi.TypeReference;
import one.contentbox.boxd.abi.datatypes.*;
import one.contentbox.boxd.abi.datatypes.generated.AbiTypes;
import one.contentbox.boxd.protocol.BoxdClient;
import one.contentbox.boxd.protocol.core.AbiDefinition;
import one.contentbox.boxd.protocol.core.Contract;
import one.contentbox.boxd.protocol.core.ObjectMapperFactory;
...
import one.contentbox.boxd.utils.Collection;
import one.contentbox.boxd.utils.Strings;
import one.contentbox.boxd.utils.Version;
...
//    private static final String WEB3J = "web3j";
    ...
    private static final String BOXD_CLIENT = "client";
    ...
    void generateJavaFiles(
        ...
        classBuilder.addMethod(buildConstructor(String.class, ADDRESS, PRIVATE_KEY, true));
        classBuilder.addMethod(buildConstructor(String.class, ADDRESS, PRIVATE_KEY, false));
        ...
        classBuilder.addMethod(buildLoad(className, String.class, ADDRESS, PRIVATE_KEY, false));
...
//            }
    ...
    List<MethodSpec> buildDeployMethods(String className,
        ...
        for (AbiDefinition functionDefinition : functionDefinitions) {
            if (functionDefinition.getType().equals("constructor")) {
                ...
                methodSpecs.add(buildDeploy(
                        className, functionDefinition, String.class, PRIVATE_KEY, ADDRESS, true));

                methodSpecs.add(buildDeploy(
                        className, functionDefinition, String.class, PRIVATE_KEY, ADDRESS, false));
            }
        }
        ...
        if (!constructor) {
            MethodSpec.Builder credentialsMethodBuilder =
                    getDeployMethodSpec(className, String.class, PRIVATE_KEY, ADDRESS,
                            false, true);
            methodSpecs.add(buildDeployNoParams(
                    credentialsMethodBuilder, className, PRIVATE_KEY, ADDRESS,
                    false, true));
            ...
            MethodSpec.Builder credentialsMethodBuilderNoGasProvider =
                    getDeployMethodSpec(className, String.class, PRIVATE_KEY, ADDRESS,
                            false, false);
            methodSpecs.add(buildDeployNoParams(
                    credentialsMethodBuilderNoGasProvider, className, PRIVATE_KEY, ADDRESS,
                    false, false));
        }
    ...
    private static MethodSpec buildConstructor(Class authType, String address, String authName,
                                               boolean withGasProvider) {
        MethodSpec.Builder toReturn = MethodSpec.constructorBuilder()
                .addModifiers(Modifier.PROTECTED)
                .addParameter(String.class, CONTRACT_ADDRESS)
                .addParameter(BoxdClient.class, BOXD_CLIENT)
        ...
        if (withGasProvider) {
            toReturn.addParameter(ContractGasProvider.class, CONTRACT_GAS_PROVIDER)
                    .addStatement("super($N, $N, $N, $N, $N, $N)",
                            BINARY, CONTRACT_ADDRESS, BOXD_CLIENT, address, authName, CONTRACT_GAS_PROVIDER);
        } else {
            toReturn.addParameter(BigInteger.class, GAS_PRICE)
                    .addParameter(BigInteger.class, GAS_LIMIT)
                    .addStatement("super($N, $N, $N, $N, $N, $N, $N)",
                            BINARY, CONTRACT_ADDRESS, BOXD_CLIENT, address, authName, GAS_PRICE, GAS_LIMIT)
    ...
    private MethodSpec buildDeploy(
            ...
            Class authType, String address, String authName, boolean withGasProvider) {

        boolean isPayable = functionDefinition.isPayable();

        MethodSpec.Builder methodBuilder = getDeployMethodSpec(
                className, authType, address, authName, isPayable, withGasProvider);
        String inputParams = addParameters(methodBuilder, functionDefinition.getInputs());

        if (!inputParams.isEmpty()) {
            return buildDeployWithParams(
                    methodBuilder, className, inputParams, address, authName,
                    isPayable, withGasProvider);
        } else {
            return buildDeployNoParams(methodBuilder, className, address, authName,
                    isPayable, withGasProvider);
        }
    }

    private static MethodSpec buildDeployWithParams(
            MethodSpec.Builder methodBuilder, String className, String inputParams, String address,
        ...
        if (isPayable && !withGasProvider) {
            methodBuilder.addStatement(
                    "return deployRemoteCall("
                            + "$L.class, $L, $L, $L, $L, $L, $L, encodedConstructor, $L)",
                    className, BOXD_CLIENT, address, authName, GAS_PRICE, GAS_LIMIT, BINARY, INITIAL_VALUE);
            methodBuilder.addAnnotation(Deprecated.class);
        } else if (isPayable && withGasProvider) {
            methodBuilder.addStatement(
                    "return deployRemoteCall("
                            + "$L.class, $L, $L, $L, $L, $L, encodedConstructor, $L)",
                    className, BOXD_CLIENT, address, authName, CONTRACT_GAS_PROVIDER, BINARY, INITIAL_VALUE);
        } else if (!isPayable && !withGasProvider) {
            methodBuilder.addStatement(
                    "return deployRemoteCall($L.class, $L, $L, $L, $L, $L, $L, encodedConstructor)",
                    className, BOXD_CLIENT, address, authName, GAS_PRICE, GAS_LIMIT, BINARY);
            methodBuilder.addAnnotation(Deprecated.class);
        } else {
            methodBuilder.addStatement(
                    "return deployRemoteCall($L.class, $L, $L,  $L, $L, $L, encodedConstructor)",
                    className, BOXD_CLIENT, address, authName, CONTRACT_GAS_PROVIDER, BINARY);
        }
    ...
    private static MethodSpec buildDeployNoParams(
            MethodSpec.Builder methodBuilder, String className, String address,
            String authName, boolean isPayable, boolean withGasPRovider) {
        if (isPayable && !withGasPRovider) {
            methodBuilder.addStatement(
    ...
    private static MethodSpec.Builder getDeployMethodSpec(
            String className, Class authType, String address, String authName,
    ...
    private static MethodSpec buildLoad(
            String className, Class authType, String address, String authName, boolean withGasProvider) {
        ...
        if (withGasProvider) {
            toReturn.addParameter(ContractGasProvider.class, CONTRACT_GAS_PROVIDER)
                    .addStatement("return new $L($L, $L, $L, $L, $L)", className,
                            CONTRACT_ADDRESS, BOXD_CLIENT, address, authName, CONTRACT_GAS_PROVIDER);
        } else {
            toReturn.addParameter(BigInteger.class, GAS_PRICE)
                    .addParameter(BigInteger.class, GAS_LIMIT)
                    .addStatement("return new $L($L, $L, $L, $L, $L, $L)", className,
                            CONTRACT_ADDRESS, BOXD_CLIENT, address, authName, GAS_PRICE, GAS_LIMIT)
                    .addAnnotation(Deprecated.class);
        }
        ...
        if (parameterSpec.type instanceof ParameterizedTypeName) {
            ...
            if (typeNames.size() != 1) {
                ...
                return "new " + parameterSpecType + "(\n"
                        + "        " + componentType + ".class,\n"
                        + "        one.contentbox.boxd.abi.Utils.typeMap("
    ...
    private void buildConstantFunction(
        ...
        if (outputParameterTypes.isEmpty()) {
            ...
            ParameterizedTypeName parameterizedTupleType = ParameterizedTypeName.get(
                    ClassName.get(
                            "one.contentbox.boxd.tuples.generated",
    ...
    private static Class<?> getStaticArrayTypeReferenceClass(String type) {
        try {
            return Class.forName("one.contentbox.boxd.abi.datatypes.generated.StaticArray" + type);
